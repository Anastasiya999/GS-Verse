...
public class SplatSegmentedDeformate : MonoBehaviour, IDeformable
{
    ...
    void Awake()
    {
        ...
        try
        {
            if (_gsVerse != null)
            {
                ...
                if (_gsVerse.originalVertices.IsCreated && _gsVerse.mesh != null)
                {
                    Initialize(_gsVerse.mesh.vertices);
                }
                else
                {
                    Debug.Log("GSVerse exists but not initialized yet; waiting for OnInitVertices.");
                }
    ...
    public void AddDeformingForce(Vector3 point, Vector3 force)
    {
        ...
        if (!displacedVertices.IsCreated || !originalVertices.IsCreated || !vertexVelocities.IsCreated)
        {
            Debug.Log($"not adding force | disp:{displacedVertices.IsCreated} orig:{originalVertices.IsCreated} vel:{vertexVelocities.IsCreated}");
            return;
        }
