...
namespace GaussianSplatting.Runtime
{
    public class SplatAccessor : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (mouseDown)
            {
                ...
                var springJob = new VertexSpringJob
                {
                    ...
                    transformedOriginalVertices = transformedOriginalVertices,
        ...
        void OnDestroy()
        {
            ...
            DisposeIfCreated(ref transformedOriginalVertices);
            DisposeIfCreated(ref displacedVertices);
            DisposeIfCreated(ref transformedDisplacedVertices);
            DisposeIfCreated(ref vertexVelocities);
            DisposeIfCreated(ref vertexVelocitiesGS);
            DisposeIfCreated(ref verticesGS);
        ...
        public struct VertexSpringJob : IJobParallelFor
        {
            ...
            public NativeArray<float3> transformedDisplacedVertices;
            public NativeArray<float3> transformedOriginalVertices;
            ...
            public void Execute(int i)
            {
                ...
                float3 velocityGS = vertexVelocitiesGS[i];
                float3 displacementGS = (transformedDisplacedVertices[i] - transformedOriginalVertices[i]) * uniformScale;
                velocityGS -= displacementGS * springForce * deltaTime;
                velocityGS *= 1f - damping * deltaTime;
                vertexVelocitiesGS[i] = velocityGS;

                displacedVertices[i] += velocity * (deltaTime / uniformScale);
                transformedDisplacedVertices[i] += velocityGS * (deltaTime / uniformScale);


                float3 t = transformedOriginalVertices[i];
                float3 additionalOffset = new float3(
                    -t.x * (deltaTime / uniformScale) * dis,
                     t.y * (deltaTime / uniformScale) * 1.5f * dis,
                     t.z * (deltaTime / uniformScale) * dis
                );

                //transformedDisplacedVertices[i] += velocityGS * (deltaTime / uniformScale) + additionalOffset;
            }
        }
        ...
        struct AddDeformingForceJob : IJobParallelFor
        {
            ...
            public void Execute(int i)
            {
                ...
                Vector3 appliedForceGS = force * attenuationGS * deltaTime;
