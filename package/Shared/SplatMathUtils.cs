...
namespace GaussianSplatting.Shared
{
    ...
    /*
     public static List<Vector3> GetMeshFaceVertices(Vector3[] meshVertices, GameObject gameObject, bool isTransform = true)
        {
            MeshFilter meshFilter = gameObject.GetComponent<MeshFilter>();
            if (meshFilter == null)
            {
          ...
          public static NativeArray<Vector3> CalculateXYZNative(
            List<Vector3> vertices,
            int numPtsEachTriangle,
            List<List<List<float>>> alphas, Allocator allocator)
        {

            int numTriangles = vertices.Count / 3;
            var normalizedAlphas = alphas;
            NativeArray<Vector3> xyz = new NativeArray<Vector3>(numTriangles * numPtsEachTriangle, allocator);
            ...
            for (int i = 0; i < numTriangles; i++)
            {
                ...
                for (int j = 0; j < numPtsEachTriangle; j++)
                {
                    var pointAlphas = triangleAlphas[j];
                    Vector3 point = (pointAlphas[0] * v0) + (pointAlphas[1] * v1) + (pointAlphas[2] * v2);
                    xyz[i * numPtsEachTriangle + j] = point;
                }
            }
